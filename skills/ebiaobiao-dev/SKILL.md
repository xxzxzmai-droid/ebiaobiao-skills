---
name: ebiaobiao-dev
description: "Route and coordinate 报表/vika/AITable development work. Use when Codex is asked to build or maintain 报表 solutions, choose between Fusion API, self-built widget mini-programs, script mini-programs, automation workflows, embedded app patterns, deployment host configuration, table creation, data processing, widget release, or reporting tools backed by vika datasheets."
---

# 报表开发总入口

The deployment host must come from local configuration. Public vika docs are syntax references only. Never expose API tokens, deployment URLs, real IDs, screenshots, commits, widget frontend code, or shared links.
...
- Embedded apps, backend gateways, scheduled jobs: `ebiaobiao-workflows`.

Load only the chosen sub-skill and needed references.

## Defaults

- Fusion API base: configured by `EBIAOBIAO_API_BASE_URL`; if omitted, derive it from `EBIAOBIAO_HOST + /fusion/v1`.
...
- Writes require `EBIAOBIAO_PROFILE=dev`, HTTPS host/base URL, token, and target space ID.

## Guardrails

- Treat "报表小程序" as vika self-built Widget unless explicitly told otherwise.
- Treat "工作站", "空间站", and "space" as target `EBIAOBIAO_SPACE_ID`.
- Run read-only discovery before writes.
- Keep business UIs click-first and responsive for phone, desktop, and embedded windows.
